...
// TODO: The PushNotificationManagerStreamSpec is copy of PushNotificationManagerProjectSpec. Try to reuse common code to test all configuration at one place if possible.
final class PushNotificationManagerStreamSpec: QuickSpec {
    ...
    override func spec() {
        ...
        beforeEach {
            ...
            exponea.configure(
                Exponea.ProjectSettings(projectToken: "mock-project-token", authorization: .token("mock-auth")),
                pushNotificationTracking: .enabled(appGroup: "mock-app-group"),
                ...
                it("should track push events in correct order when \(testCase.name)") {
                    ...
                    waitUntil(timeout: .seconds(5)) { done in
                        NetworkStubbing.stubNetwork(
                            forIntegrationType: .stream(streamId: "mock-project-token"),
                            withStatusCode: 200,
                            withDelay: 0,

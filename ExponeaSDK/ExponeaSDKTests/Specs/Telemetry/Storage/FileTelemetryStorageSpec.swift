...
final class FileTelemetryStorageSpec: QuickSpec {
    ...
    override func spec() {
        ...
        it("should not fail deleting crashlog that's not saved") {
            let storage = FileTelemetryStorage()
            storage.deleteCrashLog(CrashLog(exception: self.mockException, fatal: true, launchDate: Date()))
        }

        it("should save-get-delete crashlog") {
            let storage = FileTelemetryStorage()
            let crashLog = CrashLog(exception: self.mockException, fatal: true, launchDate: Date())
        ...
        it("should sort crashlogs") {
            let storage = FileTelemetryStorage()
            let crashLog1 = CrashLog(exception: self.mockException, fatal: true, launchDate: Date())
            usleep(5 * 1000)
            let crashLog2 = CrashLog(exception: self.mockException, fatal: true, launchDate: Date())
            usleep(5 * 1000)
            let crashLog3 = CrashLog(exception: self.mockException, fatal: true, launchDate: Date())
